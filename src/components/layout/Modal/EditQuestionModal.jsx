...
class questionsModal extends Component {
  ...
  UpdateAndClose = () => {
    ...
    if (!question || !correct || !wrong1 || !wrong2 || !wrong3) {
      document.getElementById("edited_question_error_message").innerHTML =
        "Invalid Inputs";
    ...
    if (options.length < 3) {
      document.getElementById("edited_questions_error_message").innerHTML =
        "Please check correct and wrong options!";
      this.refs.msgNotificationError.open();
      return;
    }
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response) {
          ...
          if (response !== "1") {
            document.getElementById("questions_success_message").innerHTML =
              "No changes made.";
          ...
          document.getElementById("questions_error_message").innerHTML =
            "Please try again";
          this.refs.msgNotificationError.open();
        }
      }.bind(this),
